...
from .models import Category, ShopProfile, Product, Region, User
...
SHOPS = [
    {"key": "goychay_orchard", "name": "Goychay Orchard Cooperative", "region": "Goychay", "email": "shop@bazaario.az", "phone": "+994 22 216 01 45"},
...
def seed_database():
    ...
    for region in sorted({shop["region"] for shop in SHOPS}):
    ...
    users_by_key = {}
    for shop in SHOPS:
        user = User(email=shop["email"], display_name=shop["name"], role="shop")
        user.set_password(DEMO_PASSWORDS.get(shop["email"], "ShopDemo!2026"))
        db.session.add(user)
        db.session.flush()
        db.session.add(
            ShopProfile(
    ...
    for row in SEED_PRODUCTS:
        db.session.add(
            Product(
                shop_id=users_by_key[row["shop"]].id,
                name=row["name"],
    ...
    db.session.commit()
    return {"shops": len(SHOPS), "products": len(SEED_PRODUCTS), "categories": len(ALLOWED_CATEGORIES)}
